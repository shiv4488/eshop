    ...
    if (Request.method == "POST"):
        ...
        if (p == cp):
            ...
            if (user):
                ...
                message = "Thank For Creating Buyer Account"
...
def addToCart(Request, id):
    ...
    if (cart is None):
        cart = {str(p.id): {'pid': p.id, 'pic': p.pic1.url, 'name': p.name, 'color': p.color,
                            'size': p.size, 'price': p.finalprice,
                            'qty': 1, 'total': p.finalprice, 'maincategory': p.maincategory.name,
                            'subcategory': p.subcategory.name, 'brand': p.brand.name}}
    else:
        if (str(p.id) in cart):
            item = cart[str(p.id)]
            item['qty'] = item['qty']+1
            ...
            cart.setdefault(str(p.id), {'pid': p.id, 'pic': p.pic1.url, 'name': p.name, 'color': p.color,
                                        'size': p.size, 'price': p.finalprice,
                                        'qty': 1, 'total': p.finalprice, 'maincategory': p.maincategory.name,
                                        'subcategory': p.subcategory.name, 'brand': p.brand.name})
    ...
    try:
        ...
        if (cart is not None):
            ...
            if (total < 1000 and total > 0):
                shipping = 150
        final = total+shipping
        return render(Request, 'checkout.html',{'buyer':buyer,'cart': c, 'total': total,
                                         'shipping': shipping, 'final': final})
    except:
        return redirect("/admin")
    ...
    if(Request.method == "POST"):
        ...
        if(mode=="COD"):
            ...
            if(cart is None):
                ...
                message = "Order Is Placed. Thanks For Shopping With US"
                email_from = settings.EMAIL_HOST_USER
...
def contactPage(Request):
    if(Request.method=="POST"):
        ...
        c.save()
        messages.success(Request, "Thanks For Sharing Your Issue With Us")
...
def searchPage(Request):
    ...
    return render(Request, "shop.html", {'data': data,
                  ...
                  'brand': brand, 'mc':'ALL', 'sc':'ALL', 'br':'ALL'})
...
def forgetOTP(Request):
    if(Request.method=='POST'):
        ...
        if(username):
            ...
            messages.error(Request,"UnAutharised!!!")
